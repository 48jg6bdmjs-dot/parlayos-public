...
def _run_one(name, module_path, html_path):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(name.replace(' ','_').replace('(','').replace(')',''), module_path)
...
def main():
    ...
    if not has_key:
        print("Running in OFFSEASON/DEMO mode")
    ...
    for ok, name, total, qual in results:
        ...
        print(f"  {status} {name}: {total} games")
