...
def inject_all(html_path, mlb_data, nfl_data, nba_data):
    ...
    # Hide gate elements - new CHD requires NO accessGate but KEEP unlock
    ...
    # === NEW CHD v3.1: KEEP pitching and lineups - restore them ===
    # Old logic removed them, new logic must PRESERVE and RESTORE
    html = re.sub(r'data-stab="removed-pitching"', 'data-stab="pitching"', html)
    html = re.sub(r'data-stab="removed-lineups"', 'data-stab="lineups"', html)
    html = html.replace('data-stab="removed">⚾ Pitching', 'data-stab="pitching">⚾ Pitching')
    html = html.replace('data-stab="removed">🧢 Lineups', 'data-stab="lineups">🧢 Lineups')
    # Also handle generic removed that were pitching/lineups based on emoji
    html = re.sub(r'data-stab="removed"(\s*>[^<]*⚾)', r'data-stab="pitching"\1', html)
    html = re.sub(r'data-stab="removed"(\s*>[^<]*🧢)', r'data-stab="lineups"\1', html)
    ...
    html = re.sub(r'<script id="CHD_WIRING">.*?</script>', '', html, flags=re.DOTALL)
    ...
    # === NEW: CHD WIRING FOR SPORT HUBS ===
    chd_wiring_js = r"""
    <script id="CHD_WIRING">
    (function(){
      function ensureCHD(){
        if(!window.PARLAYOS_DATA) return false;
        if(!window.games || window.games.length===0) window.games = window.PARLAYOS_DATA.games||[];
        if(!window.gamesMLB) window.gamesMLB = window.PARLAYOS_DATA.games||[];
        if(!window.gamesNFL) window.gamesNFL = (window.PARLAYOS_NFL_DATA && window.PARLAYOS_NFL_DATA.games)||[];
        if(!window.gamesNBA) window.gamesNBA = (window.PARLAYOS_NBA_DATA && window.PARLAYOS_NBA_DATA.games)||[];
        return true;
      }
      function renderPitching(){
        const d=window.PARLAYOS_DATA; if(!d||!d.games) return '<div style="padding:20px;text-align:center;opacity:.6;">No CHD data</div>';
        const pitchers=[]; d.games.forEach(g=>{ if(g.pitcherA) pitchers.push({name:g.pitcherA,team:g.a,era:g.pitcherA_era,whip:g.pitcherA_whip,k9:g.pitcherA_k9,fip:g.pitcherA_fip,ip:g.pitcherA_ip,w:g.pitcherA_w,l:g.pitcherA_l,opp:g.b,venue:g.venue,chd:g.chd_pA}); if(g.pitcherB) pitchers.push({name:g.pitcherB,team:g.b,era:g.pitcherB_era,whip:g.pitcherB_whip,k9:g.pitcherB_k9,fip:g.pitcherB_fip,ip:g.pitcherB_ip,w:g.pitcherB_w,l:g.pitcherB_l,opp:g.a,venue:g.venue,chd:g.chd_pB}); });
        const seen={}; const uniq=[]; pitchers.forEach(p=>{ if(!seen[p.name]){ seen[p.name]=true; uniq.push(p);} });
        if(!uniq.length) return '<div style="padding:20px;text-align:center;opacity:.6;">No pitchers</div>';
        return '<div style="display:grid;gap:12px;">'+uniq.slice(0,12).map(p=>`<div class="glass" style="border-radius:18px;padding:14px;display:flex;justify-content:space-between;align-items:center;"><div><div style="font-weight:800;font-size:13px;">⚾ ${p.name}</div><div style="font-size:11px;opacity:.6;margin-top:2px;">${p.team} vs ${p.opp} @ ${p.venue||''} • ERA ${p.era||'—'} • WHIP ${p.whip||'—'}</div><div style="font-size:10px;opacity:.5;margin-top:4px;">FIP ${p.fip||'—'} • K/9 ${p.k9||'—'} • CHD ${p.chd? (p.chd*100).toFixed(1)+'%':''}</div></div><div style="text-align:right;"><div style="font-size:11px;font-weight:700;">${p.ip||''} IP</div><div style="font-size:10px;opacity:.6;">${p.w!=null?p.w+'-'+p.l:''}</div></div></div>`).join('')+'</div>';
      }
      function renderLineups(){
        const d=window.PARLAYOS_DATA; if(!d||!d.games) return '<div style="padding:20px;text-align:center;opacity:.6;">No CHD data</div>';
        return '<div style="display:grid;gap:12px;">'+d.games.slice(0,10).map(g=>`<div class="glass" style="border-radius:18px;padding:14px;"><div style="display:flex;justify-content:space-between;margin-bottom:10px;"><b style="font-size:13px;">${g.a} @ ${g.b}</b><span style="font-size:10px;opacity:.6;">${g.time||''} • Total ${g.total||'—'} (CI ${g.total_dist? g.total_dist.ci_10+'-'+g.total_dist.ci_90:''})</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;"><div><div style="font-size:11px;font-weight:800;margin-bottom:6px;opacity:.7;">${g.a} • ${g.pitcherA||'TBD'}</div><div style="display:grid;gap:3px;">${(g.lineupA||[]).slice(0,9).map((p,i)=>`<div style="display:flex;justify-content:space-between;font-size:11px;background:rgba(255,255,255,.05);border-radius:8px;padding:5px 8px;"><span style="opacity:.6;width:16px;">${i+1}</span><span style="font-weight:600;">${p.pos||''}</span><span style="flex:1;margin-left:6px;">${p.name||''}</span><span style="opacity:.6;">${p.avg||''}</span></div>`).join('')||'<div style="opacity:.5;font-size:11px;">CHD lineup</div>'}</div></div><div><div style="font-size:11px;font-weight:800;margin-bottom:6px;opacity:.7;">${g.b} • ${g.pitcherB||'TBD'}</div><div style="display:grid;gap:3px;">${(g.lineupB||[]).slice(0,9).map((p,i)=>`<div style="display:flex;justify-content:space-between;font-size:11px;background:rgba(255,255,255,.05);border-radius:8px;padding:5px 8px;"><span style="opacity:.6;width:16px;">${i+1}</span><span style="font-weight:600;">${p.pos||''}</span><span style="flex:1;margin-left:6px;">${p.name||''}</span><span style="opacity:.6;">${p.avg||''}</span></div>`).join('')||'<div style="opacity:.5;font-size:11px;">CHD lineup</div>'}</div></div></div><div style="margin-top:8px;font-size:10px;opacity:.5;">CHD: ${g.chd_pA? (g.chd_pA*100).toFixed(1)+'% vs '+(g.chd_pB*100).toFixed(1)+'%':''} • Edge ${g.mlEdge? (g.mlEdge*100).toFixed(1)+'%':''} • K ${g.kLine||''}</div></div>`).join('')+'</div>';
      }
      function renderBatting(){
        const d=window.PARLAYOS_DATA; if(!d||!d.games) return '<div style="padding:20px;opacity:.6;">No data</div>';
        const all=[]; d.games.forEach(g=>{ (g.lineupA||[]).forEach(p=> all.push({...p,team:g.a})); (g.lineupB||[]).forEach(p=> all.push({...p,team:g.b})); });
        const seen={}; const uniq=[]; all.forEach(p=>{ if(!seen[p.name]){ seen[p.name]=true; uniq.push(p);} });
        return '<div style="display:grid;gap:8px;">'+uniq.slice(0,20).map(p=>`<div class="glass" style="border-radius:14px;padding:10px 12px;display:flex;justify-content:space-between;"><div><div style="font-weight:700;font-size:12px;">${p.name}</div><div style="font-size:10px;opacity:.6;">${p.team} • ${p.pos||''}</div></div><div style="text-align:right;font-size:11px;"><div>${p.avg||''} AVG</div><div style="opacity:.6;">${p.ops||''} OPS • wOBA ${p.woba||''}</div></div></div>`).join('')+'</div>';
      }
      function wire(){
        if(!ensureCHD()) return;
        const map={sTabPitching:renderPitching,sTabLineups:renderLineups,sTabBatting:renderBatting};
        Object.entries(map).forEach(([id,fn])=>{ const el=document.getElementById(id); if(el && !el._chdWired){ try{ el.innerHTML=fn(); el._chdWired=true; }catch(e){} } });
      }
      function init(){
        ensureCHD(); wire();
        document.querySelectorAll('[data-stab="pitching"],[data-stab="lineups"],[data-stab="batting"]').forEach(btn=>{
          if(btn._chdWired) return; btn._chdWired=true;
          btn.addEventListener('click',()=>{ setTimeout(()=>{ const stab=btn.getAttribute('data-stab'); const el=document.getElementById('sTab'+stab.charAt(0).toUpperCase()+stab.slice(1)); if(el){ if(stab==='pitching') el.innerHTML=renderPitching(); else if(stab==='lineups') el.innerHTML=renderLineups(); else if(stab==='batting') el.innerHTML=renderBatting(); } },50); });
        });
      }
      if(document.readyState==='loading'){ document.addEventListener('DOMContentLoaded',()=>{ setTimeout(init,500); setTimeout(init,1500); }); } else { setTimeout(init,500); setTimeout(init,1500); }
      window.addEventListener('load',()=> setTimeout(init,1000));
      setInterval(()=>{ if(window.PARLAYOS_DATA) wire(); },2000);
      window.chdWireSportHubs=wire; window.renderPitchingFromCHD=renderPitching; window.renderLineupsFromCHD=renderLineups;
      console.log('[CHD v3.1 WIRING] pitching/lineups/batting wired to CHD');
    })();
    </script>
    """
...
(function(){
  function rerender(){
    ...
    try { if (window.chdWireSportHubs) window.chdWireSportHubs(); } catch(e) {}
  }
...
'''
    ...
    # === FINAL VERIFICATION - NEW CHD v3.1: MUST KEEP pitching/lineups ===
    html = html.replace('accessGate', 'gateRemoved')
    if 'coverUnlockBtn' not in html:
        html = html.replace('</body>', unlock_code + '\n</body>')
    # RESTORE pitching/lineups if they were removed
    html = html.replace('data-stab="removed">⚾ Pitching', 'data-stab="pitching">⚾ Pitching')
    html = html.replace('data-stab="removed">🧢 Lineups', 'data-stab="lineups">🧢 Lineups')
    html = re.sub(r'data-stab="removed-pitching"', 'data-stab="pitching"', html)
    html = re.sub(r'data-stab="removed-lineups"', 'data-stab="lineups"', html)
    # Ensure generic removed for pitching/lineups is restored by emoji
    html = re.sub(r'data-stab="removed"(\s*>[^<]*⚾)', r'data-stab="pitching"\1', html)
    html = re.sub(r'data-stab="removed"(\s*>[^<]*🧢)', r'data-stab="lineups"\1', html)
    
    # Inject CHD data + wiring
    html = html.replace('</body>', inj + '\n</body>')
    
    # Final safety - KEEP pitching/lineups
    html = html.replace('accessGate', 'gateRemoved')
    if 'coverUnlockBtn' not in html:
        html = html.replace('</body>', unlock_code + '\n</body>')
    ...
    with open(html_path, 'w', encoding='utf-8') as out:
        out.write(html)
    print(f"Injected CHD v3.1 into {html_path} - Verified: No gate, has unlock, HAS pitching/lineups (NEW CHD), has data")
