"""
NFL Ace V2 - With Prop Expansion
"""
import sys

try:
    import importlib.util
    spec = importlib.util.spec_from_file_location("nfl_ace_original", "nfl_ace.py")
    orig = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(orig)
    ORIGINAL_AVAILABLE = True
except Exception as e:
    ORIGINAL_AVAILABLE = False
    print(f"NFL Original not available: {e}")
...
def run(html_path=None):
    ...
    print(f"NFL Original: {len(captured)} games")
    if PROP_AVAILABLE and captured:
        try:
            enhanced = improve_nfl_nba_with_props("NFL", captured)
            prop_count = len([p for p in enhanced if p.get('kind')=='prop'])
            print(f"NFL V2: {len(enhanced)} total ({len(captured)} games + {prop_count} props)")
