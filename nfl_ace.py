...
# V2 OVERRIDE: YT Vision disabled by default, max 2% when enabled
try:
    import json as _js_cfg
    import os as _os_cfg
    _cfg_path = _os_cfg.path.join(_os_cfg.path.dirname(__file__), "sports_config.json")
    with open(_cfg_path) as _f_cfg:
        _yt_cfg_file = _js_cfg.load(_f_cfg).get("youtube", {})
        # Only enable if explicitly enabled=true in config
...
# === V2 HELPERS - EPA/DVOA, OL/DL, Weather gated ===
...
class NFLPredictionEngine:
    ...
    def fetch_live_odds(self) -> List:
        ...
        try:
            ...
            # FIX: Filter to next 7 days only - stops 100-game dump in July
            # In July, API returns all preseason + regular season futures
            from datetime import datetime, timezone, timedelta
            ...
            # If > 20 games in 7 days, it's preseason cluster or bug - cap it
            if len(filtered) > 20:
                print(f"NFL: {len(data)} total games in API, {len(filtered)} in next 7 days - capping to 16")
                filtered = filtered[:16]
            
            # Off-season detection: July = 0 games
            if len(filtered) == 0 and len(data) > 0:
                # Check if all games are far future ( > 14 days)
                future_count = 0
                for g in data:
                    try:
                        dt = datetime.fromisoformat(g.get("commence_time","").replace("Z", "+00:00"))
                        if dt > now + timedelta(days=14):
                            future_count += 1
                    except:
                        pass
                if future_count == len(data):
                    print(f"NFL: Off-season - {len(data)} games are all >14 days out (preseason/regular season futures) - returning 0")
                    return []
            
            print(f"Odds API returned {len(data)} NFL games, {len(filtered)} within 7 days | remaining: {r.headers.get('x-requests-remaining','?')}")
            ...
            try:
                if game.get("home") and game.get("away") and "Sample" not in str(game.get("home")):
                    ...
                    if yt_cfg.get("enabled", True):
                        ...
                        if conf < 0.7 or gameplay_pct < 0.8:
                            yt_momentum = 0.0
                            yt_pace = 0.0
                        else:
                            raw_mom_capped = max(-0.02, min(0.02, raw_mom))
                            raw_pace_capped = max(-0.02, min(0.02, raw_pace))
                            yt_momentum = raw_mom_capped * conf * gameplay_pct * 0.25
                            yt_pace = raw_pace_capped * conf * gameplay_pct * 0.25
...
def run(html_path: str):
    config = load_config()
    # FIX: Use env var first, and off-season detection
    ...
    # Off-season check: July = no NFL
    from datetime import datetime
    if datetime.now().month in [6, 7]:
        print(f"  [NFL] July off-season - skipping (0 games) - will return 0, not 100")
