...
def export_to_html(games, html_path=None):
    print(f"[NFL Improved] Exporting {len(games)} games (no hardcoded key)")
    try:
        out_path=os.path.join(os.path.dirname(os.path.abspath(__file__)),"last_nfl_slate.json")
...
def export_to_html(all_games_data, html_path=None):
    ...
    here = HERE_DIR if 'HERE_DIR' in globals() else os.path.dirname(os.path.abspath(__file__))
    try:
        with open(os.path.join(here, "last_nfl_slate.json"), "w") as f:
