...
try:
    ...
    # V2: Disabled by default for performance - enable via sports_config.json youtube.enabled=true
    YT_AVAILABLE = False  # Force disabled, enable only via config
    _YT_IMPORT_OK = True
except ImportError:
    _YT_IMPORT_OK = False
    YT_AVAILABLE = False
...
# === V2: REAL LINEUP HYDRATION ===
def fetch_today_lineups_with_teams():
    """Fetch real lineups via statsapi hydrate=lineups,probablePitchers,team"""
    try:
        today = date.today().isoformat()
        url = f"https://statsapi.mlb.com/api/v1/schedule?sportId=1&date={today}&hydrate=lineups,probablePitchers,team,venue"
        r = requests.get(url, timeout=12)
        data = r.json()
        lineup_map = {}  # (away,home) -> {lineupA, lineupB, lineupA_confirmed, lineupB_confirmed}
            ...
            for g in d.get("games", []):
                ...
                try:
                    for p in g.get("lineups", {}).get("awayPlayers", []):
                        away_lineup.append({
                            "pos": p.get("position", {}).get("abbreviation", "DH"),
                            "name": p.get("fullName", "TBD"),
                            "hand": p.get("batSide", {}).get("code", "R"),
                            "avg": p.get("stats", {}).get("batting", {}).get("avg", ".250"),
                            "id": p.get("id")
                        })
                except: pass
                try:
                ...
                lineup_map[key] = {
                    "lineupA": away_lineup,
                    "lineupB": home_lineup,
                    "lineupA_confirmed": len(away_lineup) >= 8,
                    "lineupB_confirmed": len(home_lineup) >= 8,
                }
        print(f"[V2 Lineups] Fetched {len(lineup_map)} games with lineups")
        return lineup_map
    except Exception as e:
        print(f"[V2 Lineups] Error: {e}")
        return {}
...
# === TEAM BATTING FALLBACK V2 - Real 2024 aggregates with form blend ===
            ...
            try:
                if game.get("home") and game.get("away") and "Sample" not in str(game.get("home")):
                    ...
                    if yt_cfg.get("enabled", True):
                        ...
                        # V2: Gated and reduced to 2% max to avoid harming model
                        if conf < 0.7 or gameplay_pct < 0.8:
                            yt_momentum = 0.0
                            yt_pace = 0.0
                        else:
                            # Remove virality/hash noise by capping raw_mom to 0.02 max
                            raw_mom_capped = max(-0.02, min(0.02, raw_mom))
                            raw_pace_capped = max(-0.02, min(0.02, raw_pace))
                            yt_momentum = raw_mom_capped * conf * gameplay_pct * 0.25  # 25% weight
