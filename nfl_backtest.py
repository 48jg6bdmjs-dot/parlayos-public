#!/usr/bin/env python3
"""
nfl_backtest.py â€” NFL wrapper with YouTube Vision
"""
import os, sys, json
...
CSV_PATH = os.path.join(HERE, "nfl_picks_log.csv")  # Separate log per sport
...
def main(argv):
    ...
    # Fallback to mlb log if nfl log doesn't exist yet (for testing)
    csv_path = CSV_PATH
    if not os.path.exists(csv_path):
        fallback = os.path.join(HERE, "picks_log.csv")
        if os.path.exists(fallback):
            print(f"NFL log not found, using {fallback} for demo")
            csv_path = fallback
        else:
            print(f"No log found at {CSV_PATH}"); sys.exit(1)
    ...
    if not dry:
        try:
            try:
                ...
                existing = {}
            out = {**existing, **cfg}
            out["_basis"] = f"{a['totals']['bets'] if a['totals'] else 0} graded, ROI {a['totals']['roi'] if a['totals'] else 'n/a'} (auto-tuned {date.today().isoformat()})"
    ...
    if as_json:
        ...
        print_report(a, cfg, notes, wrote, "nfl")
